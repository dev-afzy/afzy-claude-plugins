# DOCX — Editing Existing Documents & XML Reference

Load this file when editing an existing `.docx` file or working with raw OOXML.
...
**Deleting entire paragraphs/list items** — also mark the paragraph mark as deleted so it merges with the next paragraph. Add `<w:del/>` inside `<w:pPr><w:rPr>`:
```xml
<w:p>
  <w:pPr>
    <w:numPr>...</w:numPr>
